---
title: For each web page, does each [refresh process](#refresh-process) (tag `<object>`, tag `<embed>`, tag `<svg>`, tag `<canvas>`, tag `<meta>`) meet one of these conditions (excluding special cases)?
steps:
