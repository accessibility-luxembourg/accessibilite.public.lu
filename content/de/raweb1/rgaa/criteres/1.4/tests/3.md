---
title: For each [button](#bouton-formulaire) of image type (tag `<input>` with the attribute `type="image"`) used as a [CAPTCHA](#captcha) or as a [test image](#image-test), having a [text alternative](#text-alternative-image), is this alternative relevant?
steps:
