---
title: Dans chaque page web, chaque image ou élément multimédia (balise `<video>`, balise `<img>`, balise `<svg>`, balise `<canvas>`, balise `<embed>` ou balise `<object>`) qui provoque un changement brusque de luminosité ou un effet de flash vérifie-t-il une de ces conditions ?
steps:
...
2. Pour chaque contenu clignotant ou provoquant des effets de flash, vérifier que :
   - soit la fréquence de l’effet est inférieure ou égale à 3 par seconde ;
   - soit la surface cumulée est inférieure à ou égale 21824 pixels.
